...
(begin-for-test
  (local
    ((define p0 (make-politician 100 200))
     ...
     (define p22 (make-politician 260 20))
     (define p23 (send p22 after-tick))
     (define p24 (make-politician 10 20))
     (define p25 (send p24 after-move 5 15))
     (define p26 (make-politician 100 100))
     (define p27 (send p26 after-tick))
     (define p28 (make-politician 100 10))
     (define p29 (send p28 after-tick))
     (define p30 (make-politician 50 5))
     (define p31 (send p30 after-tick)))
    ...
    (check-equal? (send (send p0 after-button-down 200 400)
                  ...
                  "results after executing send command twice
                 after-button-down")
    ...
    (check-equal? (send (send (new Politician%: [x 100][y 100]
                                   [mx 150][my 150][face-number 1]) after-tick)
                        toy-y) 
                  29.28932188134526)
                  "y ain't less than my. check jump value for y"
    ))